...
def main(args):
  ...
  if args.gan_loss != "none":
    if args.newDis:
      d_model = Discriminator1(dsp=args.dsp, dtp=args.dtp, dvp=args.dvp,
                            dspe=args.dspe, dtpe=args.dtpe, dvpe=args.dvpe,
                            ch=args.ch, in_re = args.in_re)
    else:
      d_model = Discriminator(dsp=args.dsp, dtp=args.dtp, dvp=args.dvp,
                            dspe=args.dspe, dtpe=args.dtpe, dvpe=args.dvpe,
                            ch=args.ch)
  ...
  for epoch in tqdm(range(args.start_epoch, args.epochs)):
    ...
    for i, sample in enumerate(train_loader):
      ...
      time_step = sample["time"].to(device)
      ...
      if args.gan_loss != "none":
        ...
        decision = d_model(sparams, vparams, time_step, image)
        ...
        fake_decision = d_model(sparams, vparams, time_step, fake_image.detach())
        ...
        time_sta = time.time()
        fake_decision = d_model(sparams, vparams, time_step, fake_image)
        time_end = time.time()
      ...
      if args.perc_loss != "none":
        ...
        # RGBチャンネルのみを抽出
        image_rgb = image[:, :3, :, :]
        fake_image_rgb = fake_image[:, :3, :, :]

        # normalize
        image_rgb = ((image_rgb + 1.) * .5 - norm_mean) / norm_std
        fake_image_rgb = ((fake_image_rgb + 1.) * .5 - norm_mean) / norm_std

        features = vgg(image_rgb)
        fake_features = vgg(fake_image_rgb)
    ...
    with torch.no_grad():
      ...
      if i == 0:
        n = min(len(sparams), 8)
        # 4次元のfake_imageを3次元のRGBと1次元のDepthに分ける
        fake_rgb = fake_image[:, :3, :, :]  # RGB部分
        fake_depth = fake_image[:, 3:, :, :]  # Depth部分

        comparison_rgb = torch.cat([image[:, :3, :, :][:n], fake_rgb[:n]])
        comparison_depth = torch.cat([image[:, 3:, :, :][:n], fake_depth[:n]])

        save_image(((comparison_rgb.cpu() + 1.) * .5),
                  os.path.join(args.root_store, "tmp",
                                str(args.mse_loss) + "_" + args.perc_loss + "_" + str(args.gan_loss) + "_" + str(epoch) + "_rgb.png"), nrow=n)
        save_image(((comparison_depth.cpu() + 1.) * .5),
                  os.path.join(args.root_store, "tmp",
                                str(args.mse_loss) + "_" + args.perc_loss + "_" + str(args.gan_loss) + "_" + str(epoch) + "_depth.png"), nrow=n)
        print("=====> Saved image")
